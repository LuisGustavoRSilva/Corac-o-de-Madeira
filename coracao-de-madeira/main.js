...
ipcMain.on('new-os', async (event, os) => {
    ...
    try {
        ...
        const newOS = new osModel({
            ...
            statusOS: os.stat_OS,
            instrumento: os.computer_OS,
            serie: os.serial_OS,
            problema: os.problem_OS,
            observacao: os.obs_OS,
            luthier: os.specialist_OS,
                ...
                try {
                    ...
                    if (dataOS && dataOS !== null) {
                        ...
                        // Extração dos dados da OS     
                            doc.text("Instrumento / Marca:", 14, 85)
                                doc.text(String(dataOS.instrumento), 55, 85)
...
async function printOS(osId) {
    try {
        ...
        // Extração dos dados da OS   
        doc.text("Instrumento:", 14, 85)                     
        doc.text(String(dataOS.instrumento), 45, 85)
...
async function relatorioOSPendentes() {
    try {
        ...
        const headers = [["Número da OS", "Entrada", "Cliente", "Telefone", "Status", "Instrumento", "Defeito"]]
        const data = []
...
async function relatorioOSFinalizadas() {
    try {
        ...
        doc.text(`Data: ${dataAtual}`, 250, 15)
        const headers = [[
            "Número da OS", "Entrada", "Cliente", "Instrumento",
            "Luthier", "Diagnóstico", "Peças", "Valor (R$)"
        ]]
        const data = []
        let totalGeral = 0
